...
vi.mock("../ToolCallBadge", () => ({
  ToolCallBadge: ({ toolName, state }: { toolName: string; state: string }) => (
    <div data-testid="tool-call-badge">{`${toolName}:${state}`}</div>
  ),
}));
...
test("MessageList renders tool-call badge for state: call (in-progress)", () => {
  const messages: Message[] = [
    {
      id: "1",
      role: "assistant",
  ...
  expect(screen.getByTestId("tool-call-badge")).toBeDefined();
  expect(screen.getByText("str_replace_editor:call")).toBeDefined();
});
